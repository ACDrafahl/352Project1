...
void cfs_scheduler(struct cpu *c) {
  ...
  // it should not be picked to run next and its vruntime should be updated
    ...
    // 3.1.6 New variable for actual increment
    int inc = cfs_proc_timeslice_len - cfs_proc_timeslice_left;
    int v_inc = (cfs_proc_timeslice_len - cfs_proc_timeslice_left) * 1024 / weight;
    // compute the increment of its vruntime according to CFS design
    if (inc < 1) {
      inc = 1; // increment should be at least 1
    }
    // Add the virtual increment to vruntime
    cfs_current_proc->vruntime += v_inc;
  ...
  if (c->proc == 0) { // when the next process should be found
    // to add:
    // 3.1.6 (1) Call shortest_runtime_proc() to find the proc with the shorestest vruntime
    // 3.1.6 (2) If (1) returns a valid process, set up cfs_current_proc, cfs_proc_timeslice_len,
    ...
    // 3.1.6 (3) If (1) returns 0, do nothing.
